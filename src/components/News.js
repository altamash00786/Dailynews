...
const News = (props) => {
    ...
    return (
        <>
            <h1 className="text-center" style={{ margin: '20px 10px' }}>Daily News - <i>Top {capitalizeFirstLetter(props.category)} Headlines</i></h1>
            {loading && <Spinner />}
            <InfiniteScroll
                dataLength={articles ? articles.length : 0} // Check if articles is defined
                next={fetchMoreData}
                hasMore={articles && articles.length !== totalResults} // Check if articles is defined
                loader={<Spinner />}
            >
